...
class FlipPaste:
    ...
    def __init__(self, p=0.5):
        self.p = p
        self.flip = Albu([dict(type="HorizontalFlip", p=1)],
                          keymap=keymap,
    ...
    def __call__(self, input):
        ...
        new_masks.masks = np.vstack([input["gt_masks"].masks,
                                     flip_image["gt_masks"].masks])
        ...
        input["gt_semantic_seg"] = np.logical_or(input["gt_semantic_seg"], flip_image["gt_semantic_seg"]).astype(np.uint8)
