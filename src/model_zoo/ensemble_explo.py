        ...
        for m in range(1000):
            ...
            for i, (wrapper, model) in enumerate(zip(self.wrappers, self.models)):
                ...
                for fts, img_meta in zip(features[i], img_metas):
                    ...
                    plt.subplot(1, 5, k + 1)
                    plt.imshow(mask_plot[m][cell_type] > 0.45)
                    plt.title(f'Model {i} - flip = {str(flip_direction)}')
            ...
            plt.subplot(1, 5, 5)
            plt.imshow(merged_masks[m][cell_type] > 0.45)
            plt.show()

            if len(mask_plot) == m + 1 or m > 6:
                break
            # break
